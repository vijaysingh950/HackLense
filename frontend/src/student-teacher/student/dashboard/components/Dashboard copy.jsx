...
const Dashboard = ({ addNotification, updateAssignments }) => {
  ...
  const togglePin = (assignmentId) => {
    setAssignments(prev => prev.map(a => 
      a.id === assignmentId ? { ...a, isPinned: !a.isPinned } : a
    ));
    
    addNotification(
      assignments.find(a => a.id === assignmentId).isPinned 
        ? { message: 'Assignment unpinned', type: 'info' }
        : { message: 'Assignment pinned to top', type: 'info' }
    );
  };
  ...
  // Calculate completion percentage for gamification
        ...
        <div className="dashboard">
          <div className="dashboard-header fade-in">
            <div className="header-left">
              ...
              <p className="subtitle">Welcome back! You've completed {completionPercentage}% of your assignments.</p>
            </div>
          ...
          <div className="assignments-header fade-in">
            <div className="header-left">
              <h2>Assignments</h2>
              <p className="result-count">{filteredAssignments.length} assignment{filteredAssignments.length !== 1 ? 's' : ''} found</p>
            </div>
            <div className="header-right">
          ...
          <div className={`assignments-list ${viewMode}`}>
            {sortedAssignments.length > 0 ? (
              sortedAssignments.map((assignment, index) => (
                <div 
                  key={assignment.id} 
                  className={`assignment-card fade-in ${assignment.status} ${isOverdue(assignment.deadline) && assignment.status === 'pending' ? 'overdue' : ''} ${assignment.isPinned ? 'pinned' : ''}`}
