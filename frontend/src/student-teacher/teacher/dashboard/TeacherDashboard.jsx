...
const TeacherDashboard = ({ addNotification }) => {
  ...
  useEffect(() => {
    ...
    if (filterSubject !== "all") {
      filtered = filtered.filter((problem) => problem.subject === filterSubject)
    }
        ...
        <div className="dashboard">
          <div className="dashboard-header fade-in">
            <div className="header-left">
              ...
              <p className="subtitle">Welcome back! You've created {stats.total} problems for your students.</p>
            </div>
            ...
            <div className="header-right">
              <div className="view-controls">
                <button
                  ...
                  title="Grid View">
                  <div className="view"><ion-icon name="apps-outline"></ion-icon></div>
                </button>
                <button
                  className={`view-mode-btn ${viewMode === "list" ? "active" : ""}`}
                  onClick={() => setViewMode("list")}
                  title="List View">
                 <div className="view"><ion-icon name="list"></ion-icon></div>
                </button>
              </div>
          ...
          <div className={`assignments-list ${viewMode}`}>
            {sortedProblems.length > 0 ? (
              sortedProblems.map((problem, index) => (
                <div
                  key={problem.id}
                  className={`assignment-card fade-in ${problem.status} ${isOverdue(problem.deadline) && problem.status === "active" ? "overdue" : ""} `}
                  ref={(el) => {
                    problemRefs.current[problem.id] = el
                  }}
                  style={{ "--card-index": index, "--card-color": problem.color }}
                >
                  <div className="assignment-header" style={{ backgroundColor: problem.color }}>
                  ...
                  <div className="assignment-details">
                    ...
                    <div className="assignment-meta">
                      ...
                      <span
                        className={`deadline ${isOverdue(problem.deadline) && problem.status === "active" ? "overdue" : ""}`}
                      >
                        {isOverdue(problem.deadline) && problem.status === "active" ? "OVERDUE: " : "Due: "}
                        {formatDate(problem.deadline)}
                        {!isOverdue(problem.deadline) && problem.status === "active" && (
                          <span className="days-remaining">
                            ({getDaysRemaining(problem.deadline)} day
                            {getDaysRemaining(problem.deadline) !== 1 ? "s" : ""} left)
                          </span>
                        )}
                      </span>
                    </div>
                    ...
                    <div className="submission-stats">
                      <div className="submission-header">
                        ...
                        <span className="submission-count">
                          {problem.submissionCount || 0} / {problem.totalStudents || 30}
                        </span>
                      </div>
                      <div className="submission-progress">
                        <div
                          ...
                          style={{
                            width: `${problem.submissionCount ? (problem.submissionCount / (problem.totalStudents || 30)) * 100 : 0}%`,
                      ...
                      <p className="submission-status">
                        {problem.submissionCount
                          ? `${problem.submissionCount} student${problem.submissionCount !== 1 ? "s have" : " has"} submitted their solution${problem.submissionCount !== 1 ? "s" : ""}`
                          : "No submissions yet"}
                      </p>
                    ...
                    <button className="view-button" onClick={() => viewProblem(problem.id)}>
                        View Submissions
                    </button>
